...
import { ShoppingCart, Leaf, Phone, Mail, Instagram, Facebook, Youtube, Sparkles, Check, Palette } from "lucide-react";
...
const BASE_PRICES = { wreath: { small: 45, medium: 65, large: 95 }, centerpiece: { small: 35, medium: 55, large: 85 }, memorial: { small: 40, medium: 70, large: 110 } };
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
        ...
        <nav className="hidden items-center gap-6 md:flex">
          {nav.map((n) => (
            <a key={n.href} href={n.href} className="text-sm font-medium text-neutral-700 hover:text-neutral-900">{n.label}</a>
          ))}
...
function MobileNav() {
  ...
  return (
    <>
      ...
      <div id="mobile-drawer" data-open={open ? "true" : "false"} className="fixed inset-0 z-[60]">
        <div className="absolute inset-0 bg-black/40" onClick={() => setOpen(false)} aria-hidden="true" />
        <div className="absolute right-0 top-0 h-full w-80 bg-white shadow-2xl p-6" role="dialog" aria-modal="true">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button type="button" aria-label="Close menu" onClick={() => setOpen(false)}
              className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-neutral-300 bg-white text-neutral-800">✕</button>
          </div>
          <nav className="flex flex-col gap-4">
            {nav.map((n) => (
              <a key={n.href} href={n.href} className="text-base" onClick={() => setOpen(false)}>{n.label}</a>
            ))}
            <button type="button" className="mt-2 inline-flex items-center gap-2 rounded-xl border border-transparent bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-emerald-700" onClick={() => setOpen(false)}>
  ...
  return (
    <section className="relative isolate overflow-hidden bg-gradient-to-br from-emerald-50 via-lime-50 to-amber-50">
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-4 py-20 md:grid-cols-2 md:py-28">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight md:text-5xl">Welcome to <span className="text-emerald-700">{brand.name}</span></h1>
        ...
        <div className="aspect-square w-full rounded-3xl bg-[url('https://images.unsplash.com/photo-1542990253-0d0f5be5f0ed?q=80&w=1200&auto=format&fit=crop')] bg-cover bg-center shadow-2xl" aria-label="Photo of a decorative wreath" />
      </div>
    </section>
  );
}
...
function CustomBuilder() {
  ...
  function handleSubmit(e){ e.preventDefault(); alert('Thanks! Your request was received.'); e.currentTarget.reset(); }
...
function Stat({ number, label }) {
  return (<Card><CardContent className="p-6 text-center"><div className="text-3xl font-extrabold tracking-tight">{number}</div><div className="mt-1 text-sm text-neutral-600">{label}</div></CardContent></Card>);
}
function FAQ() {
  const faqs = [
    ...
    <section id="faq" className="bg-neutral-50">
      <div className="mx-auto max-w-7xl px-4 py-16">
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
          {faqs.map((f, i) => (
            <Card key={i}><CardHeader className="pb-2"><CardTitle className="text-base">{f.q}</CardTitle></CardHeader><CardContent><p className="text-neutral-700">{f.a}</p></CardContent></Card>
          ))}
        </div>
      </div>
    </section>
  ...
  return (
    <footer className="border-t bg-white">
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 py-10 md:grid-cols-4">
        <div><div className="flex items-center gap-2"><Leaf className="h-5 w-5"/><span className="font-semibold">{brand.name}</span></div><p className="mt-2 text-sm text-neutral-600">{brand.tagline}.</p></div>
        <div><h4 className="font-semibold">Shop</h4><ul className="mt-2 space-y-1 text-sm text-neutral-700"><li><a href="#shop">Wreaths</a></li><li><a href="#shop">Centerpieces</a></li><li><a href="#shop">Memorial</a></li></ul></div>
        <div><h4 className="font-semibold">Learn</h4><ul className="mt-2 space-y-1 text-sm text-neutral-700"><li><a href="#tutorials">Tutorials</a></li><li><a href="#faq">FAQ</a></li><li><a href="#custom">Custom orders</a></li></ul></div>
        <div><h4 className="font-semibold">Contact</h4><ul className="mt-2 space-y-1 text-sm text-neutral-700"><li className="flex items-center gap-2"><Phone className="h-4 w-4"/> {brand.phone}</li><li className="flex items-center gap-2"><Mail className="h-4 w-4"/> {brand.email}</li></ul></div>
      </div>
